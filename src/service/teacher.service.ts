...
export const teacherService = {
  async getTeachers() {
    const res = await apiConfig().getRequest(ApiUrls.TEACHERS,);
    return res;
  },
  ...
  async updateTeacher(model: Teacher) {
    const { id } = model;
    model = {
    ...
    console.log(model);
    const res = await apiConfig().updateRequest(`${ApiUrls.TEACHERS}/${id}`,model);
    // Notification("success", res?.data.message);
